...
Yes we can by using another RoT(Root of Trust), the [[teaproject/original-root/obsidian-backup/_tapp-tutor/TPM]] chip.

## Proof of Trusted Computing
TEA Project is a project that relies on a hardware RoT. Every TEA node needs to have the [[teaproject/original-root/obsidian-backup/_tapp-tutor/TPM]] chip built-in. The TPM chip uses Trusted Computing technology to collect the evidence of hardware integrity. That TPM data is verified by other TEA nodes (verifiers) via [[Remote_Attestation]] (RA). The decision is made by the verifier independently, and final judgement is done by the blockchain's BFT. If there's any change in a TEA node, no matter hardware or software, it will be detected by a verifier and furthermore marked "malicious" in the blockchain. Other TEA nodes will immediately reject communication with this bad actor. 
...
## Learn more
Please go to [[conveyor]] and [[teaproject/original-root/obsidian-backup/_tapp-tutor/State_Machine]] to learn more.

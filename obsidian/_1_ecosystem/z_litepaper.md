...
### 1. The TEA Utility Token
Our first token, [[_token/tea-tokens/README|TEA]], is a utility token with a supply of 100 million tokens (hard cap / no inflation) pre-allocated in the genesis block. TEA is also given as a reward to miners for running public services (e.g. remote attestation) on the TEA network. 

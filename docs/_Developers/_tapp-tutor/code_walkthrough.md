...
* [party-fe](party-fe.md): This is the [front_end](front_end.md).
...
Querying the state can return the result without having to wait in conveyor queue. But the communication is still async, so additional queries for more results are still needed which is not shown in the diagram. You can see the details on additional queries at [party-fe](party-fe.md)'s workflow section.
...
The following diagram demonstrates the workflow of how a simple transfer txn command is handled. Note that this diagram is a simplifed verison. The full version can be found in [party-fe](party-fe.md)'s workflow section.
